...
export default function WorkspacePage() {
  ...
  const customers = useQuery({
    queryKey: ["workspace-customers", search, advisorFilter],
    queryFn: async () =>
      (
        await api.get<Paginated<Customer>>("/customers", {
          params: {
            search: search || undefined,
            advisor_id: advisorFilter ? Number(advisorFilter) : undefined,
            page_size: 50,
          },
        })
      ).data,
  ...
  return (
    <div>
      ...
      <div className="panel mb-4 grid gap-3 p-4 sm:grid-cols-2">
        ...
        >
          <option value="">كل المستشارات</option>
          {(advisorNames.data ?? []).map((a) => (
            <option key={a.id} value={a.id}>
              {a.name}
              {a.advisor_code ? ` (${a.advisor_code})` : ""}
            </option>
          ))}
        </select>
      </div>
          ...
          <table className="data">
            ...
            <tbody>
              ...
              {!customers.isLoading && customers.data?.items.length === 0 ? (
                <tr>
                  <td colSpan={9}>لا توجد استمارات بعد.</td>
                </tr>
              ) : null}
            </tbody>
          </table>
